...
#### Script for 2004-2006 web survey starts here ####

# set working directory to 'instdir'.
setwd(instdir)

# assign zipped dataset file to 'data_file' variable.
data_file <- here::here("inst/extdata/CES-E-2004-2006.zip")

# unzip 'data_file' and place unzipped folder in 'instdir' file location.
unzip(data_file, exdir = here::here("inst/extdata/ces04-06"))

# assign directory within the unzipped folder.
instdir_main <- here::here("inst/extdata/ces04-06")
...
for(i in 1:length(files)){
  ces0406 <- haven::read_sav(file = files[i])
}
...
save(ces0406, file = "ces0406.RData")
