...
demoecon <- function(data = ces::onlinesurvey){
  ...
  mutate(lr_bef = as.character(lr_bef)) %>%                                     # reassign values in lr_bef column as characters for uniting
  mutate(lr_aft = as.character(lr_aft)) %>%                                     # reassign values in lr_aft column as characters for uniting
  unite("lr", lr_bef:lr_aft, na.rm = TRUE, remove = FALSE) %>%                  # unite lr_bef and lr_aft columns into new column lr
  mutate(yob = as.integer(yob)) %>%                                             # reassign values in yob as integers for recoding below
  mutate(yob = recode(yob,                                                      # recode survey code years for actual year of birth
                      ...
                      `91` = "2010")) %>%
  mutate(income_cat = as.integer(income_cat)) %>%                               # reassign values in income_cat as integers for recoding
  mutate(income_cat = recode(income_cat,                                        # recode survey code income categories for actual income categories
  ...
  mutate(province_territory = as.character(province_territory)) %>%             # reassign values in province_territory as characters for recoding
  mutate(province_territory = recode(province_territory,                        # recode survey code province and territory categories to internationally
                                                                                # approved alpha codes
